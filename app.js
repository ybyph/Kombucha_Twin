...
function updateF1Prediction(result) {
    try {
        if (window.f1Locked) {
            const pt = document.getElementById('f1-prediction-text');
            if (pt) pt.innerText = '一发已锁死，等待装瓶二发';
            const cal = document.getElementById('add-to-calendar-f1');
        ...
        if (timeDiffHours <= 0 || isNaN(timeDiffHours)) {
            const pt = document.getElementById('f1-prediction-text');
            if (pt) pt.innerText = '等待更多数据录入...';
            const cal = document.getElementById('add-to-calendar-f1');
            if (cal) cal.classList.add('hidden');
        ...
        if (brixDropRate <= 0 || isNaN(brixDropRate)) {
            const pt = document.getElementById('f1-prediction-text');
            if (pt) pt.innerText = '发酵动力不足，等待更多数据...';
            const cal = document.getElementById('add-to-calendar-f1');
            if (cal) cal.classList.add('hidden');
            return;
        }
        ...
        if (brixToDrop <= 0) {
            const pt = document.getElementById('f1-prediction-text');
            if (pt) pt.innerText = '已达黄金风味点，建议立即装瓶!';
            const cal = document.getElementById('add-to-calendar-f1');
            if (cal) {
        ...
        const d = Math.floor(hoursToTarget / 24);
        const h = Math.floor(hoursToTarget % 24);
        const m = Math.floor((hoursToTarget % 1) * 60);
        
        let predictionText = '预计黄金装瓶窗口：';
        if (d > 0) predictionText += `${d} 天 `;
        if (h > 0) predictionText += `${h} 小时 `;
        predictionText += `${m} 分钟后`;
        
        const pt = document.getElementById('f1-prediction-text');
        if (pt) pt.innerText = predictionText;
        const cal = document.getElementById('add-to-calendar-f1');
        if (cal) {
            cal.classList.remove('hidden');
            cal.onclick = function() {
        ...
        console.error('updateF1Prediction error:', e);
    }
}

function updateEndF1Button(result) {
    try {
        const btn = document.getElementById('end-f1-btn');
        if (!btn) return;
        
        if (window.f1Locked) {
            btn.style.display = 'none';
            const hint = document.getElementById('f1-locked-hint');
            if (hint) hint.classList.remove('hidden');
            return;
        }
        
        const hasTwoLogs = engine.logs.length >= 2;
        const safeAbv = result && isFinite(result.abv) ? result.abv : 0;
        const hasValidAbv = safeAbv > 0;
        const hasValidRemaining = result && result.remainingHours !== undefined && isFinite(result.remainingHours) && result.remainingHours > 0;
        ...
        console.error('updateEndF1Button error:', e);
    }
}
...
function startF2Countdown() {
    try {
        ...
        console.error('startF2Countdown error:', e);
    }
}
...
function calculateF2Pressure() {
    try {
        ...
        if (!window.f2CountdownStarted) {
            safeSetText('f2-time-remaining', '待启动');
            return;
        }
        ...
        if (fruitWeight > maxFruitWeight) {
            ...
            if (ratio < 5) {
                if (fruitHint) { fruitHint.textContent = '气泡可能较弱'; fruitHint.style.color = '#fbbf24'; }
                fruitInput.style.backgroundColor = '';
                fruitInput.style.borderColor = '';
            } else if (ratio > 20) {
                if (fruitHint) { fruitHint.textContent = '纤维过多，建议减量'; fruitHint.style.color = '#fbbf24'; }
                fruitInput.style.backgroundColor = '';
                fruitInput.style.borderColor = '';
            } else {
        ...
        if (extraSugar === 0 && co2Volumes < 1.5 && totalSugar > 0) {
            if (sugarHint) { sugarHint.textContent = '燃料不足，建议加 2-4g 砂糖以产生气泡'; sugarHint.classList.remove('hidden'); sugarHint.style.color = '#fbbf24'; }
        } else if (extraSugar > 15) {
            if (sugarHint) { sugarHint.textContent = '警告：糖分过多，请减少补糖以防炸瓶'; sugarHint.classList.remove('hidden'); sugarHint.style.color = '#ef4444'; }
        }
        ...
        window.f2CountdownInterval = setInterval(() => {
            ...
            if (remaining <= 0) {
                safeSetText('f2-time-remaining', '已达标!');
                const el = document.getElementById('f2-time-remaining');
                if (el) el.className = 'text-xl mono font-bold text-green-400';
                clearInterval(window.f2CountdownInterval);
                return;
            }
...
function startNewBatch() {
    if (confirm('确定要开始新批次吗？这将清空所有历史记录和配置。')) {
        ...
        document.getElementById('vessel-d').value = '100';
        document.getElementById('liquid-h').value = '10';
        ...
        const tanninHint = document.getElementById('f2-tannin-hint');
        if (tanninHint) { tanninHint.classList.add('hidden'); tanninHint.style.display = 'none'; }
        ...
        safeSetText('f2-chill-status', '气泡锁定中，请勿开瓶');
        safeSetText('f2-chill-timer', '24:00:00');
        safeSetText('f2-time-remaining', '待启动');
        ...
        if (dangerHint) dangerHint.classList.add('hidden');
        const sugarHint = document.getElementById('f2-sugar-hint');
        if (sugarHint) { sugarHint.classList.add('hidden'); sugarHint.textContent = ''; }
        const fruitHint = document.getElementById('f2-fruit-hint');
        if (fruitHint) fruitHint.textContent = '';
        safeSetText('f2-liquid-weight', '0 g');
        safeSetText('f2-total-sugar', '0 g');
        ...
        if (hc) hc.innerHTML = '';
        safeSetText('kinetic-efficiency', '1.00x');
...
function initEventListeners() {
    ...
    document.getElementById('end-f1-btn').onclick = () => {
        if (engine.logs.length < 2) {
...
window.onload = () => {
    ...
    if (window.f2Activated) {
        ...
        if (window.f2CountdownStarted) {
            const cdb = document.getElementById('f2-start-countdown-btn');
            if (cdb) cdb.classList.add('hidden');
            const calendarBtn = document.getElementById('add-to-calendar-f2');
            const chillBtn = document.getElementById('f2-start-chill');
            if (calendarBtn) { calendarBtn.disabled = false; calendarBtn.classList.remove('opacity-50', 'cursor-not-allowed'); }
            if (chillBtn) { chillBtn.disabled = false; chillBtn.classList.remove('opacity-50', 'cursor-not-allowed'); }
